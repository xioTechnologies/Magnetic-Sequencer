...
RCON_RESET_CAUSE ResetCauseGet() {
    ...
    const RCON_RESET_CAUSE mask = RCON_RESET_CAUSE_POR | RCON_RESET_CAUSE_BOR | RCON_RESET_CAUSE_IDLE | RCON_RESET_CAUSE_SLEEP | RCON_RESET_CAUSE_WDTO | RCON_RESET_CAUSE_SWR | RCON_RESET_CAUSE_EXTR | RCON_RESET_CAUSE_VREGS | RCON_RESET_CAUSE_CMR | RCON_RESET_CAUSE_HVDR;
...
void ResetCausePrint(const RCON_RESET_CAUSE cause) {
    ...
    if ((cause & RCON_RESET_CAUSE_VREGS) != 0) {
        printf("VREGS ");
    }
    if ((cause & RCON_RESET_CAUSE_CMR) != 0) {
        printf("CMR ");
    }
    if ((cause & RCON_RESET_CAUSE_HVDR) != 0) {
        printf("HVDR ");
    }
